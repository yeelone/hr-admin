...
export class EmployeeComponent implements OnInit {
  ...
  topParentGroup:string = "0";
  moveGroupParent:string = "" ; // 记录调动的父类，如部门，岗位，学历 
  moveTopGroup:string = "0"; //在调动弹窗里选择的调动类型
  moveToNewGroupID:number ;
  remark:string = "";//创建、更新、删除时必须加入备注
  total:number ;
  queryFreezed:string = "false"; //请求冻结的，或者是激活的
  ...
  getSelectedNode(event:Group):void{
    ...
    if (this.targetGroup) {
      if ( this.targetGroup.parent === 0 ) {
        ...
        let level = this.targetGroup.levels.split(".");
